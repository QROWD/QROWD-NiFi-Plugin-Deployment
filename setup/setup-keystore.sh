...
keytool -importkeystore -srcstorepass "$KEYSTORE_PASSWORD" -deststorepass "$KEYSTORE_PASSWORD" -destkeypass "$KEYSTORE_PASSWORD" -srckeystore "$target/admin.pfx" -keystore "$target/keystore.jks"
